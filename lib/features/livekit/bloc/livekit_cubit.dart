...
class LiveKitCubit extends Cubit<LiveKitState> {
  ...
  Future<void> init({
    ...
    try {
      ...
      // =========================================
      final micEnabled = await _enableMicrophone(
        localParticipant: localParticipant,
        microphonePermission: microphonePermission,
      );

      // =========================================
      // ENABLE CAMERA AFTER MICROPHONE
      // =========================================
      final cameraEnabled = await _enableCamera(
        localParticipant: localParticipant,
        cameraPermission: cameraPermission,
      );

      print('🎤 FINAL MIC STATUS => $micEnabled');
      print('📷 FINAL CAMERA STATUS => $cameraEnabled');
  ...
  // ENABLE MICROPHONE
  // =========================================
  Future<bool> _enableMicrophone({
    required LocalParticipant localParticipant,
    required PermissionStatus microphonePermission,
  }) async {
    ...
    try {
      ...
      final isMicOn = localParticipant.isMicrophoneEnabled();

      print('🎤 MICROPHONE ENABLED => $isMicOn');

      return isMicOn;
    } catch (e) {
      print('⚠️ MICROPHONE ENABLE ERROR => $e');
  ...
  // =========================================
  // ENABLE CAMERA WITH RETRY
  // =========================================
  Future<bool> _enableCamera({
    required LocalParticipant localParticipant,
    required PermissionStatus cameraPermission,
  }) async {
    ...
    try {
      ...
      final isCameraOn = localParticipant.isCameraEnabled();

      print('📷 CAMERA STATUS => $isCameraOn');

      return isCameraOn;
    } catch (e, stackTrace) {
      print('❌ CAMERA ENABLE ERROR => $e');
      print('❌ STACK TRACE => $stackTrace');
  ...
  void _handleJoinLeft() {
    ...
    final localParticipant = currentRoom.localParticipant;
    ...
    for (final participantId in currentParticipantIds) {
      if (!_previousParticipantIds.contains(participantId)) {
        print(
    ...
    for (final participantId in _previousParticipantIds) {
  ...
  void _emitParticipants() {
    ...
    emit(
      LiveKitConnected(
        ...
        isCameraOff: !localParticipant.isCameraEnabled(),
      ),
    );
  }
