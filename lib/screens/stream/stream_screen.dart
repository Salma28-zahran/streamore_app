...
class _StreamScreenState extends State<StreamScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // Background content (Column, Tabs, and Controls)
          Column(
            children: [
              Padding(
                padding: const EdgeInsets.only(top: 15, left: 8, right: 8),
                child: Center(
                  child: GestureDetector(
                    ...
                    child: Stack(
                      alignment: Alignment.center,
                      children: [
                        // Profile image (always on top of the overlay)
                        Container(
                          width: profileImageWidth,
                          height: profileImageHeight,
                          decoration: BoxDecoration(
          ...
          // Overlay widget (Below the profile logo)
          Consumer<MyProvider>(
            builder: (context, provider, child) {
              if (!provider.isOverlayVisible) return SizedBox();
          ...
          // Profile image widget (Above overlay)
          Consumer<MyProvider>(
            builder: (context, provider, child) {
              if (!provider.isLogoVisible) return SizedBox();
          ...
          // Fullscreen profile view (Always on top)
          if (_isFullScreen)
            GestureDetector(
              onTap: () {
                setState(() {
